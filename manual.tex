...
现存的研究中很少有直接使用包到达间隔分布作为VoIP识别的特征，在一篇使用机器学习进行VoIP流分类的研究中\supercite{5}，作者使用VoIP语音流的包到达间隔的标准差作为一项特征进行识别。另外，一篇进行VoIP流量统计的文章中显示，在VoIP信令控制流量和媒体传输流量中70\%的包到达间隔少于20ms，意味着VoIP应用对语音的打包周期普遍少于20ms。
